...
* Look at the Display.java **interface** and in your own words, write below why you think we are declaring an **interface**.
  * the Display interface is an interface because not just is it to teach us about the amazing things you can do in java, it is also because we can set up multiple "displays" (possibly with unique actions when monitoring) to keep track of the weather.
